...
class Spotify:
    ...
    def __inti__(self):
        self.api_url = "https://accounts.spotify.com/authorize"
        self.encoded_data = base64.b64encode(bytes(f"{secrets.SPOTIFY_CLIENT_ID}:{SPOTIFY_CLIENT_SECRET}", "ISO-8859-1"))
    ...
    def get_auth_url(self):
        ...
        auth_code_dict = {
            "client_id": secrets.SPOTIFY_APP_ID,
            "response_type": "code",
        ...
        return [self.api_url + "oauth/?" + params, state]
    ...
    def get_access_token(self, code):
        ...
        response = requests.post(
            self.api_url + "v1/oauth/token", data=data, headers=self.authorization_header_string
        )
        response_data = json.loads(response.text)
        try:
            access_token = response_data["access_token"]["url"]
